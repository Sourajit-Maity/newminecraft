...
(function ($) {
  ...
  $container.append(
    '<h5>Customize IFB-Agro Dashboard</h5><hr class="mb-2"/>'
  )

  // Default Font Size
  var font_html = '<h6>Font Size</h6>'+
    '<div class="d-flex">'+
      '<select id="body_font_selector" class="custom-select mb-3 text-light border-0 bg-white">'
  for(let i = 8; i < 27; i++){
    font_html += '<option>'+ i +'</option>'
  }
  font_html += '</select>'+
    '</div>'

  $container.append(font_html)
  
  // Fetching font size from localstorage
  var default_body_font = localStorage.getItem("font_size")
  console.log('default_body_font: ',default_body_font)

  // Setting default value if not exists
  if(default_body_font == null || default_body_font == undefined){
    default_body_font = 14

    localStorage.setItem("font_size", default_body_font)
  }

  // Applying font size
  $('body').css('font-size', parseInt(default_body_font))
  $('#body_font_selector').val(default_body_font)

  $('#body_font_selector').change(function(e){
    default_body_font = $(this).val()
    localStorage.setItem("font_size", default_body_font)

    $('body').css('font-size', parseInt(default_body_font))

    console.log('Current Font Size: ', default_body_font)
  })

  // Theme settings
  var theme_header_html = '<h6>Theme</h6>'
  $container.append(theme_header_html)

  var dark_mode_status = localStorage.getItem("dark_mode")

  // Setting default value if not exists
  if(dark_mode_status == null || dark_mode_status == undefined){
    dark_mode_status = 0
    localStorage.setItem("dark_mode", dark_mode_status)
  }
