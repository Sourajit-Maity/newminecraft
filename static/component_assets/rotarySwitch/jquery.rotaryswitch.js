...
(function($, window, document, undefined) {
	...
	function Plugin(element, options) {
		...
		this.htmlStructure = {
			...
			mark: '<div class="mark">'+defaults.input+'</div>'
		};
		...
		*/
		...
		renderMarks: function() {
			...
			for (; i < len; i += 1) {
				...
				var mark = $(this.htmlStructure.mark).css({'transform': 'rotate('+deg+'deg) translate(-6px, -'+ (this.domElements.main.width()/2 + (this.domElements.main.width()*0.4)) +'px)','background':'#FFF'});
				// var mark ='h'
				marks.append(mark);
				deg += degPerStep;
			}
